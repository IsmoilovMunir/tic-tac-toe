...
public class Main {
    ...
    private static String GAME_STATE_O_WON = "0 победили!";
    private static String GAME_STATE_DRAW = "Ничья";
    private static String GAME_STATE_NOT_FINISHED = "Игра не закончено";
    ...
    public static void startGemLoop(String[][] board) {
        ...
        do {
            ...
            String gmeState = checkGameState(board);
            if (!Objects.equals(gmeState, GAME_STATE_NOT_FINISHED)) {
                System.out.println(gmeState);
                return;
            }
    ...
    public static int[] inputCellCoordinates(String[][] board) {
        System.out.println("Введите 2 числа (ряд и колонку) от 0 до 2 координаты через пробел (0-2): ");
        // допущения - не проверяем на наличе пробела, и корректность цифр
        do {
    ...
    public static void makeBotTurn(String[][] board) {
        ...
        int[] coordinates = gerRandomEmptyCellCoordinates(board);
        board[coordinates[0]][coordinates[1]] = CELL_STATE_O;
    }

    public static int[] gerRandomEmptyCellCoordinates(String[][] board) {
        do {
    ...
    public static String checkGameState(String[][] board) {
        ...
        // diagonal from to-left to bottom - right
